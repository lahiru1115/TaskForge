...
**Part B — done:** `workspace.controller.ts` (CRUD + members) and `invite.controller.ts` (create/list/revoke/accept), routed through an expanded `workspace.routes.ts` plus a new top-level `invite.routes.ts` for `POST /api/invites/:token/accept` (not nested under `:slug` — the token identifies the workspace, and the caller isn't a member yet). `listUsers`/`GET /api/users` is deleted outright — replaced by `GET /api/workspaces/:slug/members` (paginated, `?q=` prefix search via a `User` lookup, active members only).

Judgment calls made while building this, none spelled out in the original plan:
- **Workspace delete cascades**: owner-only, deletes every `Task`/`Comment`/`Activity`/`WorkspaceMember`/`Invite` scoped to it before deleting the workspace itself. "CRUD" implied a real delete, not a stub.
- **Member removal allows self-removal** ("leave a workspace") without requiring owner/admin rights on yourself — only removing *someone else* requires `requireWorkspaceRole('owner', 'admin')`. The workspace owner can never be removed or have their role changed through these endpoints.
- **Role-assignment endpoints (member-role-update, invite-creation) never accept `'owner'`** — ownership transfer isn't a feature here, so it can't be granted through a side door.
- **Invite tokens**: 32 random bytes, only the SHA-256 hash persisted, raw token returned once in the create response (email delivery is Phase 3). Accept checks `expiresAt` explicitly rather than relying on the TTL index, since Mongo's TTL sweep runs on its own ~60s cycle, not synchronously at read time.
...
- [x] **Seed rewrite:** two workspaces — "Acme Product" (all four users: admin=owner, jane=admin, john=member, sarah=**viewer**) and "Side Project" (admin=owner, jane=member only) — plus one pending invite (`mike@taskforge.com`, unaccepted).
  ...
  **Follow-up (readability):** the ~500-line inline data (task titles/descriptions, comment threads) was pulled out of `seed.ts` into `src/scripts/seed-data/*.json` (`users`, `workspaces`, `memberships`, `tasks.acme`, `tasks.side`, `comments`, `invites`). Cross-references (a task's `createdBy`, a comment's `taskIndex`) use plain string keys resolved to real ObjectIds at seed time, since the JSON is authored before any document exists to reference. What stayed as code: password hashing, per-workspace fractional-index rank generation, and the activity-timeline simulation (how long a task sits in each status before advancing) — that's algorithm, not data. Re-verified after the split: `npm run seed` output matched the pre-split run exactly (same counts), and a live API check confirmed the first Acme task's title/assignee/comments resolved correctly from the JSON.
