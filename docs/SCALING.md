...
- [x] **Step 2 — new pages:** `pages/Workspaces.tsx` (list + create, own header — not under the existing `Navbar`, matching how `Login`/`Register` are full-page too), `pages/AcceptInvite.tsx`, `pages/settings/Members.tsx` (search, invite, role change, remove/leave, pending invites). Plus `components/workspace/CreateWorkspaceDialog.tsx` and `InviteMemberDialog.tsx`. All existing shadcn/ui primitives (Card, Dialog, Select, Field, AlertDialog, Badge, Skeleton) covered every need — no new component installs. Consume Step 1; not routed yet.
  - Fixed a real type-safety gap surfaced while wiring this up: `useUpdateMember`/`useCreateInvite` correctly reject `'owner'` at the type level (`AssignableRole = Exclude<WorkspaceRole, 'owner'>`, matching the backend's own validation), and the first draft of the role-change `<Select>` handler wasn't narrow enough — caught by `tsc`, not a runtime bug, but a good sign the type is actually doing its job.
  - Also moved `CreateWorkspaceInput`/`CreateInviteInput` into `lib/schemas.ts` (Zod-validated) rather than leaving them as plain interfaces in the hook files, matching how `useTasks.ts` already sources `TaskInput` from there — the workspace slug field now gets the same client-side format validation the backend enforces, instead of only failing after a round trip.
  - **Verification limit, stated plainly:** build/lint/typecheck all pass and the components follow established patterns closely (same form/dialog/error-handling shapes as `CreateTaskDialog`/`Login`/`Trash`), but none of this is routed yet, and this tool can't render a browser — so it's unverified visually. Real verification happens once Step 4 wires routing and there's something to actually load in a browser.
- [x] **Step 3 — `WorkspaceSwitcher`** component (`components/layout/WorkspaceSwitcher.tsx`): Popover + the existing `components/ui/command.tsx`, standard shadcn combobox shape — trigger shows the current workspace name, popover lists all memberships with a checkmark on the active one and each member's role, plus a "Manage workspaces" row (inside the same `CommandList`, not floated outside it, so it stays part of cmdk's registered item set) that routes to `/workspaces`. Reads the active workspace from `useCurrentWorkspace()` — only ever renders correctly once mounted inside `WorkspaceProvider`, which happens in Step 4. Not yet mounted in `Navbar.tsx`. Build/lint clean; same visual-verification limit as Step 2 — nothing to load in a browser until Step 4.
- [x] **Step 4 — routing rewrite:** new `components/layout/WorkspaceLayout.tsx` — resolves `:slug` via `WorkspaceProvider`, shows a skeleton while loading, and renders the same "not found" treatment for a nonexistent workspace *and* a workspace the caller isn't a member of (never distinguishes the two, matching the backend's 404-not-403 policy) via `Outlet` otherwise. Also stamps the resolved slug into `localStorage` (`tf-last-workspace`) on every successful load. `App.tsx` restructured: `/login`, `/register`, `/invite/:token` stay top-level; `/workspaces` sits behind `ProtectedRoute` but outside any workspace shell (it has its own header, from Step 2); `/w/:slug/*` nests `WorkspaceLayout` → the existing `AppLayout` (Navbar + CommandPalette) → the existing pages, plus the new Members page at `/w/:slug/settings/members`; `/` resolves via a `RootRedirect` reading that same localStorage key, falling back to `/workspaces`. `WorkspaceSwitcher` is now mounted in `Navbar.tsx`, next to the logo.
  - **Scope call:** the plan described this step as "mount WorkspaceSwitcher in Navbar," but doing only that would leave routing internally coherent while every *other* component that built an absolute path (`NAV_ITEMS`, the active-tab-highlighting regex, task links in `TaskCard`/`TaskTable`/`BoardCard`/`Calendar`/`DayOverflowDialog`, the "back to tasks" links in `Dashboard`/`TaskDetail`, and every navigation command in `CommandPalette`) kept pointing at the old unscoped URLs. Swept all of it in this step via `grep` for every `navigate('/...')` / `to="/..."` / `to={` /...\`}` call site — 12 files — rather than discovering them one broken link at a time later. Added a `Members` entry to `NAV_ITEMS` and `CommandPalette`'s nav group too, since the page now has a real route.
  - `Navbar.tsx`'s `NAV_ITEMS` now hold slug-relative suffixes (`''`, `'tasks'`, `'settings/members'`, …), prefixed with `/w/${slug}` at render time via a small `workspacePath()` helper; the task-detail active-tab regex became `` /^\/w\/${slug}\/tasks\/[^/]+$/ ``.
  - Build and lint both pass with no new warnings (`WorkspaceContext.tsx`'s pre-existing `only-export-components` warning from Step 1 is the only workspace-related one, unchanged).
  - **Honest limit, sharper than in prior steps:** routing itself can be exercised in a browser now, and I still can't do that — no render/screenshot tool. But even once someone does load it, `Tasks`/`Board`/`Calendar`/`TaskDetail` will route correctly and then fail to load data: `useTasks.ts` still calls `/api/tasks` (unscoped), which the completed backend phase already removed in favor of `/api/workspaces/:slug/tasks`. That gap is Step 5, not a bug in this step. `CommandPalette`'s task-search cache lookup is left reading the old `['tasks']` query key for the same reason — changing it now would just search an empty cache since nothing writes to `['ws', slug, 'tasks']` yet.
- [x] **Step 5 — query key + endpoint migration:** `useTasks.ts`, `useComments.ts`, and `useActivity.ts` now call `/api/workspaces/:slug/tasks/...` and key every query/mutation off `['ws', slug, ...]` (`['ws', slug, 'tasks', filters]`, `['ws', slug, 'tasks', 'stats']`, `['ws', slug, 'trash']`, `['ws', slug, 'comments', taskId]`, `['ws', slug, 'activity', taskId]`), matching the convention `useMembers.ts` set in Step 1. Every hook that needs a workspace now takes `slug` as its first explicit parameter (not read from context internally), for the same reason `useMembers(slug, …)` already does it that way — consistency, and it keeps the hook usable from anywhere without assuming a `WorkspaceProvider` is mounted above every possible caller. `useUsers.ts` is deleted; `/api/users` (global "list every user") no longer exists on the backend.
  - **Full ripple, not just the three hook files:** every call site of a task/comment/activity hook needed its `slug` threaded through — 13 files (`Tasks`, `Trash`, `Calendar`, `TaskDetail`, `Dashboard`, `KanbanBoard`, `CreateTaskDialog`, `EditTaskDialog`, `CommentSection`, `ActivityFeed`, `BulkActionBar`, `Board`, plus `CommandPalette`'s cached-task search, which now reads `['ws', slug, 'tasks']` instead of the stale `['tasks']` key flagged as a known gap in Step 4). Each pulls `slug` from `useCurrentWorkspace()`, so this only works because every one of these components already renders under `WorkspaceLayout` after Step 4.
  - **`useUsers()` → `useMembers()` swap (4 files: `TaskForm`, `FilterBar`, `BulkActionBar`, `Board`):** assignee dropdowns previously read a flat `UserOption[]` from the now-deleted global endpoint; they now call `useMembers(slug, { limit: 100 })` and derive `users = (data?.members ?? []).map(m => m.user)`, matching the shape (`_id`/`name`/`email`) the existing dropdown code already expected — no changes needed below that line in any of the four files. **Scope call:** `limit: 100` is a pragmatic ceiling (the backend's max), not real pagination — fine for this app's workspace sizes (seed data tops out at 4 users/workspace), but a workspace with >100 members would silently truncate the assignee list. Worth a comment if this project ever needs to support that.
  - This is the real live-verification checkpoint: build and lint both pass clean (identical baseline warnings, nothing new), and — unlike Steps 2–4 — every page in the app now has a correct, workspace-scoped path all the way from route → data hook → API call. I still can't render a browser myself, so this remains asserted from reading the code and passing typecheck, not from clicking through it.
- [x] **Step 6 — `AuthContext.tsx` rename + audit:** `isAdmin` → `isPlatformAdmin` on `AuthContextValue`, with a doc comment pinning down what it actually means now (`User.role`, platform staff only, grants no tenant access). New `hooks/useWorkspaceRole.ts` — `{ role, isManager }`, `isManager = role === 'owner' || role === 'admin'` — mirrors `isWorkspaceManager` in the backend's `services/authz.ts` exactly, so the frontend and backend definitions can't quietly drift apart. Audited all 13 files that read `isAdmin` (the 14th, `useUsers.ts`, was already deleted in Step 5) and split them three ways instead of one:
  - **Stayed platform-level, renamed only** (3 files): `Navbar.tsx`'s "Admin" badge, `ProfileAvatar.tsx`'s badge, `ProfileDetails.tsx`'s "Administrator"/"Member" row. All three display the caller's own platform account type, not a permission decision — correctly unrelated to any workspace.
  - **Moved to `useWorkspaceRole()`, same shape as before** (3 files): `TaskDetail.tsx` and `BulkActionBar.tsx`'s `canManage = isManager || task.createdBy._id === user?._id` now match the backend's `canManage()` in `authz.ts` precisely — previously they checked `isAdmin`, which was already wrong pre-Step-1 in spirit and flatly incorrect post-Step-1, since task authorization has read only `WorkspaceMember.role` since the backend phase landed. `CommentSection.tsx`'s `canDelete = isOwn || isManager` matches `deleteComment`'s `isAuthor || isWorkspaceManager` — deliberately **no** creator fallback, since comment deletion and task management are different checks on the backend and conflating them would grant a task's creator delete rights over teammates' comments they never had.
  - **Gate removed entirely, not renamed** (5 files: `Trash.tsx`, `TaskTable.tsx`, `FilterBar.tsx`, `Board.tsx`, `TaskForm.tsx`) — this is the one real judgment call in this step, worth stating plainly. These gated the assignee column/filter/dropdown and the trash "· by {creator}" note behind `isAdmin`, from the single-tenant world where only admins could see every task and everyone else only saw their own. Post-Phase-1, `scopeFilter` already returns every task in the workspace to every role — assignee data was already present on every task response reaching every member, the frontend was just hiding a column/filter that carried no confidentiality value. Rather than swap the gate to `isManager` (which would newly hide it from `member`/`viewer` for no backend-backed reason), removed the gate — matches what the API already exposes to everyone. `TaskForm.tsx` additionally had a *second*, redundant layer: the outer `statusOnly` prop (driven by the caller's real `canManage` check) already decided whether non-status fields render at all, so the inner `isAdmin` ternary around just the "Assign to" field was dead weight once `statusOnly` was false — collapsed to unconditional.
  - Build and lint both pass with no new warnings (identical baseline set to Step 5).
  - **Phase 1 is now complete.** All six frontend steps plus the backend work are done: workspaces are a structural boundary from routing through data hooks through authorization, and the frontend's permission checks match the backend's `authz.ts` definitions field-for-field instead of by convention.
